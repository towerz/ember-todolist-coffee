...
var paths = {
  files: ['src/**/*.js', 'src/**/*.coffee', 'src/**/*.css', 'src/**/*.scss'],
  main: ['src/index.js'],
  tests: ['test/**/*.js'],
  dest: 'bin'
};
...
gulp.task("jst", ["sass"], function() {
  var templates = _(glob('src/**/*.html')).map(format);
  var styles = _(glob('src/**/*.css')).map(format);
...
gulp.task("build", ["assets", "jst", "coffee"], function () {
  return gulp.src(paths.main, { read: false })
